...
입학 면접 대비 서비스 — 기본 대상: **KAIST 신입생 선발 면접**. 생활기록부·자기소개서
PDF를 업로드하면 Google Gemini가 예상 질문을 추출하고, 실시간 압박 면접
시뮬레이션을 제공합니다.

대상 전형 표시명은 secrets의 `TARGET_EXAM` 키로 바꿀 수 있어 과학고 등 다른
전형에도 같은 코드로 대응합니다.

> ⚠️ **면접관의 실제 페르소나·질문 스타일·평가 기준은 코드가 아니라 secrets의
> `PROMPT_SECRET`이 결정합니다.** KAIST용으로 운영하려면 프롬프트도 KAIST 면접
> 기준(수학/과학/영어 역량, 전공 적합성, 학업 계획 등)으로 작성해야 합니다.
...
# 선택: 대상 전형 표시명 (미지정 시 "KAIST 신입생 선발 면접")
# TARGET_EXAM = "KAIST 신입생 선발 면접"
