...
if st.session_state.simulation_mode:
    ...
    st.divider()
    st.subheader("⚙️ 프리미엄 기능 (9,900원)")

    # 1. 세션 상태에 'premium_unlocked' 확인
    if not st.session_state.get('premium_unlocked', False):
        
        with st.expander("프리미엄 기능 잠금 해제하기 (클릭)", expanded=False):
            st.markdown("""
            ### 🚀 프리미엄 기능 잠금 해제 (9,900원)
            
            '면접관 AI'의 모든 잠재력을 끌어내어 합격 시나리오를 완성하세요.
            프리미엄 기능은 1회 분석(현재 업로드된 서류 기준)에 대해 유효합니다.
            따라서 종료전 ctrl + S로 현재 페이지를 저장해두시길 권장드립니다.
            **페이지 종료 후 재접속 시 프리미엄 기능이 잠금 해제되지 않습니다.**
            **저장하지 않아서 생기는 문제는 사용자에게 있음을 명확히 고지합니다.**
            
            **[프리미엄 기능 포함 내역]**
            * 🔬 **심층 해부 질문 (20개):** 서류의 모든 단어와 문장을 파고드는 정밀 타격 질문
            * 📑 **프리미엄 종합 전략 보고서:** 합격 시나리오와 4D 전략 분석 (강점/약점/기회/위협)
            * 💡 **전략적 모범 답안 패키지:** 생성된 모든 질문에 대한 완벽한 방어 논리 제공
            * 🤖 **실시간 압박 면접 시뮬레이션:** 
            ---
            
            #### **[결제 진행 방법]**
            
            **1. 아래 계좌로 9,900원을 입금해주세요.**
            * **은행:** [토스뱅크]
            * **계좌번호:** [1001-2570-7899]
            * **예금주:** [조준영]
            
            **2. 입금자명 확인**
            * 원활한 확인을 위해, 입금자명은 **[본인 실명]**으로 설정해주세요.
            
            **3. 액세스 코드 발급 요청**
            * 입금 완료 후, 개발자에게 코드 입력을 요청하세요.
            
            **4. 코드 입력**
            * 개발자가 입금 확인 후 **[액세스 코드]**를 입력해드립니다.
            * 아래 입력창에 코드를 입력하면 즉시 모든 프리미엄 기능이 활성화됩니다.
            
            ---
            """)
            
            # 2. 비밀번호(액세스 코드) 입력창
            entered_code = st.text_input("받으신 액세스 코드를 입력하세요", type="password")
            
            if st.button("코드 확인 및 활성화", use_container_width=True):
                try:
                    # 3. st.secrets에 저장된 실제 코드와 비교
                    # (주의: .streamlit/secrets.toml 파일에 PREMIUM_ACCESS_CODE = "실제코드" 추가 필요)
                    if entered_code == st.secrets["PREMIUM_ACCESS_CODE"]:
                        st.session_state.premium_unlocked = True
                        st.success("🎉 프리미엄 기능이 활성화되었습니다! 페이지가 새로고침됩니다.")
                        st.balloons()
                        st.rerun() # UI 즉시 업데이트
                    else:
                        st.error("코드가 일치하지 않습니다. 입금 확인 및 코드 발급을 다시 확인해주세요.")
                except KeyError:
                    st.error("앱 관리자 오류: PREMIUM_ACCESS_CODE가 secrets.toml에 설정되지 않았습니다.")
                except Exception as e:
                    st.error(f"오류 발생: {e}")
